...
object SMERT {
  ...
  def iteration(nbests : RDD[Tuple2[Matrix, IndexedSeq[BleuStats]]], projection : Matrix) = {
    ...
    val line = swept.flatMap(identity).cache
    val collected = line.collect
  ...
  def iterate(prevPoint : Vector, prevBleu : (Double, Double), nbests : RDD[Tuple2[Matrix, IndexedSeq[BleuStats]]], deltaBleu : Double) : (Vector, (Double, Double))= {
    val updates = for {
      d <- (0 until prevPoint.size).par
      projection = createProjectionMatrix(d, prevPoint)
    } yield iteration(nbests, projection)
    val (point, (bleu, bp)) = updates.maxBy(_._2)
  ...
  def main(args: Array[String]) : Unit = {
    case class Config(nbestsDir: File = new File("."), initialPoint: Vector = DenseVector(), localThreads: Option[Int] = None, noOfPartitions : Int=100, deltaBleu : Double = 1.0E-4, random : Random = new Random(11l), noOfInitials : Int = 1000)
